...
// ─── Helpers ─────────────────────────────────────────────────────────────────
static cv::Mat applyBrightnessContrast(const cv::Mat &src, int bright,
                                       int cont) {
...
// ─── Detection overlay ───────────────────────────────────────────────────────
static void drawDetections(cv::Mat &canvas, const std::vector<Detection> &dets,
                           const cv::Mat &disp16, double focal, double baseline,
                           int maxDepthMm) {
  for (const auto &d : dets) {
    ...
    float depthMm = 0.0f;
    if (!disp16.empty() && ctr.x >= 0 && ctr.x < disp16.cols && ctr.y >= 0 &&
        ctr.y < disp16.rows) {
      int16_t dv = disp16.at<int16_t>(ctr.y, ctr.x);
      if (dv > 8)
        depthMm = static_cast<float>(focal * baseline / (dv / 16.0));
    }
    if (maxDepthMm > 0 && depthMm > 0 && depthMm > maxDepthMm)
      continue;
    ...
    char txt[128];
    if (depthMm > 0)
      std::snprintf(txt, sizeof(txt), "%s %.0fcm %.0f%%", d.label.c_str(),
                    depthMm / 10.0f, d.confidence * 100.0f);
    else
      std::snprintf(txt, sizeof(txt), "%s %.0f%%", d.label.c_str(),
                    d.confidence * 100.0f);

    int baseline_ = 0;
    cv::Size ts =
        cv::getTextSize(txt, cv::FONT_HERSHEY_SIMPLEX, 0.45, 1, &baseline_);
    ...
    cv::putText(canvas, txt, {b.x + 2, b.y - 2}, cv::FONT_HERSHEY_SIMPLEX, 0.45,
                cv::Scalar(0, 0, 0), 1);
  }
}
...
int main(int argc, char **argv) {
  ...
  if (!arEnabled)
    std::cout << "[AR] Modelos no encontrados. Ejecuta: bash "
  ...
  cv::Mat dispSmooth; // mapa de disparidad suavizado temporalmente

  // Manual calibration scale
  std::cout << "\nTeclas: C = calibrar escala  |  S = grabar/detener AVI  |  "
               "ESC = salir\n\n";
  ...
  for (;;) {
    ...
    if (!frameL.empty() && !frameR.empty()) {
      ...
      if (dtMs <= SYNC_MS) {
        ...
        // Disparity heatmap con suavizado temporal (reduce parpadeo)
        cv::Mat newDisp = makeDisparityHeatmap(disp, sp.numDisp);
        if (dispSmooth.empty() || dispSmooth.size() != newDisp.size())
          dispSmooth = newDisp.clone();
        else
          cv::addWeighted(newDisp, 0.4, dispSmooth, 0.6, 0, dispSmooth);
        dispColor = dispSmooth;
        ...
        if (detector.isLoaded()) {
          ...
          if (!dispColor.empty())
            drawDetections(dispColor, dets, disp, stereo.focalPx(),
                           stereo.baselineMm(), maxMm);
        }
    ...
    if (renderNeeded && !frameL.empty()) {
      cv::Mat left =
          applyBrightnessContrast(frameL, g_ctrl.brightness, g_ctrl.contrast);
      ...
      // Hand detection on display-resolution frame so coords match render
      // target
      if (arEnabled)
      ...
      if (zFiltCm > 1.0f)
        std::snprintf(hud, sizeof(hud), "Z raw: %.1f cm   Z filt: %.1f cm",
                      zRawCm, zFiltCm);
      else
        std::snprintf(hud, sizeof(hud), "Z: ---");

      cv::putText(canvas, hud, {10, DISP_H - 12}, cv::FONT_HERSHEY_SIMPLEX,
                  0.65, cv::Scalar(0, 0, 0), 3);
      cv::putText(canvas, hud, {10, DISP_H - 12}, cv::FONT_HERSHEY_SIMPLEX,
                  0.65, cv::Scalar(0, 255, 0), 1);

      // Crosshair center
      cv::Point ctr(DISP_W / 4, DISP_H / 2);
      cv::drawMarker(canvas, ctr, cv::Scalar(0, 0, 0), cv::MARKER_CROSS, 18, 3);
      cv::drawMarker(canvas, ctr, cv::Scalar(0, 255, 0), cv::MARKER_CROSS, 18,
                     1);

      // Recording indicator
      if (recording) {
        cv::circle(canvas, {DISP_W - 20, 20}, 8, cv::Scalar(0, 0, 255),
                   cv::FILLED);
        writer.write(canvas);
      }
        ...
        std::string fname =
            ...
            std::to_string(
                std::chrono::duration_cast<std::chrono::seconds>(
                    ...
                    .count()) +
            ".avi";
        writer.open(fname, cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 20,
